...
Innerhalb des inneren Match-Statements überprüfen wir, ob die linke Unterexpression ein Int-Objekt den Wert 0 hat. Wenn dies der Fall ist, wird sofort der Wert 0 zurückgegeben, da das Ergebnis der Multiplikation mit 0 immer 0 ist und wir uns so das Evaluieren des rechten Ausdrucks sparen.
...
Mittels des impl Blocks ist es sehr einfach, neue Funktionalitäten für Enums (oder auch Structs) bereitzustellen, ohne diese selbst zu ändern.
Im Beispiel fügen wir die neue Funktion treeHeight zum Enum hinzu, die die Tiefe unseres Expression-Trees errechnet.
\begin{minted}{rust}
pub enum Exp {
    Int {
        val: i32
...
Als generischen Typen kann man einfach die Klasse selbst übergeben. Der einzige Nachteil ist, dass die Information reduntant erscheint.
\begin{minted}{java}
...
\section{Vergleich}
\subsection{Enums}
Rust deckt alle Funktionalitäten von Java Enums ab und bietet darüber hinaus mit algebraischen Datentypen und Pattern Matching mächtige Programmierkonzepte. In Java ist es zwar möglich, über Klassen eine ähnliches Maß an Funktionalität zu erreichen, der Code ist aber weitaus verboser und weniger lesbar.
Es ist nicht möglich, abgeschlossenes Pattern Matching und Nested Pattern Matching in Java über Switch-Case zu realisieren. Switch-Case lässt insbesondere keine komplexen Datentypen zu.
Um Funktionalität von Enums zu erweitern, müssen wir in Java Enums selbst anpassen, wohingegen wir in Rust vollkommen entkoppelt Funktionen für das Enum bereitstellen können.
...
Zusammenfassend kann man also sagen, dass Java zwar oft ans gleiche Ziel kommt, aber unnötig verbos und anfälliger für Fehler ist.
